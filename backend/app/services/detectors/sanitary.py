"""
Sanitary network detection and analysis using Apryse + LLM.

This module provides functions to detect sanitary sewer network elements using:
...
logger.info(f"Sanitary detector: MIN_CONFIDENCE threshold = {MIN_CONFIDENCE}")
...
def detect_sanitary_network(vectors: List[Dict], texts: List[Dict], pdf_path: str | None = None) -> Dict[str, Any]:
    """
    Detect sanitary network using Apryse + LLM pipeline.
    ...
    if USE_DEMO:
        logger.info("ESTIMAI_USE_DEMO=1: Using demo data for sanitary network")
        nodes = _demo_detect_nodes(texts)
        pipes = _demo_trace_edges(nodes)
        return _attach_labels_and_qa(pipes, texts, "sanitary")
    
    # Real pipeline
    logger.info("Detecting sanitary network using Takeoff AI Agent")
    ...
    try:
        ...
        # Layer hints for sanitary
        layer_hints = ["SANITARY", "SAN", "SEWER", "SANITARY SEWER", "C-SAN"]
        ...
        # Log raw LLM results
        logger.info(f"Sanitary LLM: {len(patches)} patches → {len(detections)} detections")
        ...
        for i, d in enumerate(detections):
            ...
            should_include, reason = should_include_in_network(d, "sanitary", patch_data)
            
            # Try post-classification rules if no match
            if not should_include and (d.attrs.discipline is None or d.attrs.discipline != "sanitary"):
                ...
                if assigned_discipline == "sanitary":
                    d.attrs.discipline = "sanitary"
                    should_include = True
            ...
            logger.debug(f"Including {d.polyline_id} in sanitary network: {reason}")
            if assignment_method:
                assignment_stats[assignment_method] = assignment_stats.get(assignment_method, 0) + 1
            sanitary_detections.append(d)
        
        logger.info(
            f"AFTER filter (discipline='sanitary', conf>={MIN_CONFIDENCE}): {len(sanitary_detections)} pipes "
            f"(avg confidence: {sum(d.attrs.confidence for d in sanitary_detections) / len(sanitary_detections) if sanitary_detections else 0:.2f})"
        )
        logger.info(
            f"📊 Sanitary assignment breakdown: "
        ...
        if len(sanitary_detections) == 0:
            logger.warning(
                f"⚠️ 0 sanitary pipes after classification (from {len(patches)} candidates, {len(detections)} detections). "
        ...
        for i, detection in enumerate(sanitary_detections):
            polyline = next((p for p in polylines if p.id == detection.polyline_id), None)
            if not polyline:
                continue
            
            from_id = f"manhole_{len(nodes)}"
            to_id = f"manhole_{len(nodes) + 1}"
            ...
            if polyline.points and len(polyline.points) >= 2:
                ...
                if from_id not in node_ids:
                    nodes.append({"id": from_id, "kind": "manhole", "x": from_pt[0], "y": from_pt[1], "attrs": {}})
                    node_ids.add(from_id)
                
                if to_id not in node_ids:
                    nodes.append({"id": to_id, "kind": "manhole", "x": to_pt[0], "y": to_pt[1], "attrs": {}})
                    node_ids.add(to_id)
            ...
            pipe_dict = {
                ...
                "to_id": to_id,
                "length_ft": polyline.length_ft,
                "dia_in": detection.attrs.dia_in or 8.0,
        ...
        logger.info(f"Extracting elevations for {len(pipe_dicts)} sanitary pipes...")
        ...
        for i, pipe_dict in enumerate(pipe_dicts):
            detection = sanitary_detections[i] if i < len(sanitary_detections) else None
            polyline = None
            if detection:
                polyline = next((p for p in polylines if p.id == detection.polyline_id), None)
        ...
        for pipe_dict in pipe_dicts:
            qa_flags.extend(validate_pipe_qa(pipe_dict, "sewer"))
        
        logger.info(f"Sanitary network: {len(nodes)} nodes, {len(pipe_dicts)} pipes, {len(qa_flags)} QA flags")
        ...
        if USE_DEMO:
            logger.error(f"Real sanitary detection failed: {e}", exc_info=True)
            logger.warning("Falling back to demo data (ESTIMAI_USE_DEMO=1)")
            nodes = _demo_detect_nodes(texts)
            pipes = _demo_trace_edges(nodes)
            return _attach_labels_and_qa(pipes, texts, "sanitary")
        else:
            logger.error(f"❌ Real sanitary detection failed and ESTIMAI_USE_DEMO=0: {e}", exc_info=True)
...
def _attach_labels_and_qa(pipes: List[Pipe], texts: List[Dict], discipline: str) -> Dict[str, Any]:
    ...
    nodes = [{"id": f"manhole_{i}", "kind": "manhole", "x": 0.0, "y": 0.0, "attrs": {}} for i in range(len(pipes) + 1)]
