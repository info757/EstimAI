"""Sanitary sewer network detection and extraction."""
import re
import math
import uuid
...
def detect_nodes(vectors: List[VectorEl], texts: List[TextEl], symbol_map: Dict[str, Any]) -> List[Node]:
    """
    Detect sanitary sewer network nodes from vectors and texts.
    ...
    # Get sanitary-related symbols from symbol map
    sanitary_symbols = _get_sanitary_symbols(symbol_map)
    
    # Detect manholes
    for vector in vectors:
        if _is_sanitary_manhole(vector, sanitary_symbols):
            node = _create_node_from_vector(vector, NodeType.MANHOLE)
            if node:
                nodes.append(node)
    
    # Detect cleanouts
    for vector in vectors:
        if _is_cleanout(vector, sanitary_symbols):
            node = _create_node_from_vector(vector, NodeType.JUNCTION)
            if node:
                nodes.append(node)
    
    # Detect junctions (intersections of sanitary lines)
    junctions = _detect_junctions(vectors, sanitary_symbols)
    nodes.extend(junctions)
    
    logger.info(f"Detected {len(nodes)} sanitary sewer nodes")
    return nodes


def trace_edges(vectors: List[VectorEl], symbol_map: Dict[str, Any]) -> List[Edge]:
    """
    Trace sanitary sewer network edges from vectors.
    ...
    # Find sanitary pipe vectors
    sanitary_pipes = [v for v in vectors if _is_sanitary_pipe(v, sanitary_symbols)]
    
    # Merge connected polylines
    merged_pipes = _merge_connected_polylines(sanitary_pipes)
    ...
    logger.info(f"Traced {len(edges)} sanitary sewer edges")
    return edges


def attach_labels(edges: List[Edge], texts: List[TextEl]) -> List[Edge]:
    """
    Attach labels to sanitary sewer edges.
    ...
    for edge in edges:
        ...
        # Parse labels for diameter, slope, etc.
        parsed_labels = _parse_sanitary_labels(nearby_texts)
        ...
        if parsed_labels:
            ...
            if "slope_percent" in parsed_labels:
                edge.slope_percent = parsed_labels["slope_percent"]
            if "slope_ratio" in parsed_labels:
                edge.slope_ratio = parsed_labels["slope_ratio"]
...
def _get_sanitary_symbols(symbol_map: Dict[str, Any]) -> Dict[str, Any]:
    """Get sanitary-related symbols from symbol map."""
    sanitary_symbols = {}
    
    if "symbols" not in symbol_map:
        return sanitary_symbols
    
    symbols = symbol_map["symbols"]
    
    # Look for sanitary-related symbols
    sanitary_keywords = ["sanitary", "sewer", "manhole", "cleanout", "pipe", "conduit"]
    
    for symbol_name, symbol_data in symbols.items():
        symbol_name_lower = symbol_name.lower()
        layer_hints = symbol_data.get("layer_hint", [])
        
        # Check if symbol is sanitary-related
        is_sanitary = any(keyword in symbol_name_lower for keyword in sanitary_keywords)
        is_sanitary_layer = any("sanitary" in layer.lower() for layer in layer_hints)
        
        if is_sanitary or is_sanitary_layer:
            sanitary_symbols[symbol_name] = symbol_data
    
    return sanitary_symbols


def _is_sanitary_manhole(vector: VectorEl, sanitary_symbols: Dict[str, Any]) -> bool:
    """Check if vector represents a sanitary manhole."""
    # Check against sanitary symbols
    for symbol_name, symbol_data in sanitary_symbols.items():
        if "manhole" in symbol_name.lower() and _matches_symbol(vector, symbol_data):
            return True
    
    # Check for common manhole patterns
...
def _is_cleanout(vector: VectorEl, sanitary_symbols: Dict[str, Any]) -> bool:
    """Check if vector represents a cleanout."""
    # Check against sanitary symbols
    for symbol_name, symbol_data in sanitary_symbols.items():
        if "cleanout" in symbol_name.lower() and _matches_symbol(vector, symbol_data):
            return True
    
    # Check for cleanout patterns (small circles or squares)
    if vector.kind == "circle" and vector.stroke_w > 0:
        # Check size (cleanouts are typically smaller than manholes)
        bbox = _get_vector_bbox(vector)
        if bbox:
            width = bbox[2] - bbox[0]
            height = bbox[3] - bbox[1]
            if width < 10.0 and height < 10.0:  # Small size threshold
                return True
    
    return False


def _is_sanitary_pipe(vector: VectorEl, sanitary_symbols: Dict[str, Any]) -> bool:
    """Check if vector represents a sanitary pipe."""
    # Check against sanitary symbols
    for symbol_name, symbol_data in sanitary_symbols.items():
...
def _detect_junctions(vectors: List[VectorEl], sanitary_symbols: Dict[str, Any]) -> List[Node]:
    """Detect junctions where sanitary lines intersect."""
    junctions = []
...
def _parse_sanitary_labels(texts: List[TextEl]) -> Dict[str, Any]:
    """Parse sanitary sewer labels from text elements."""
    labels = {}
    ...
    for text in texts:
        ...
        # Parse slope (e.g., "2%", "1:50", "0.02")
        slope_match = re.search(r'(\d+(?:\.\d+)?)\s*%', text_content)
        if slope_match:
            labels["slope_percent"] = float(slope_match.group(1))
        
        # Parse slope ratio (e.g., "1:50", "1/50")
        ratio_match = re.search(r'(\d+(?:\.\d+)?)\s*[:/]\s*(\d+(?:\.\d+)?)', text_content)
        if ratio_match:
            rise = float(ratio_match.group(1))
            run = float(ratio_match.group(2))
            if run > 0:
                labels["slope_ratio"] = rise / run
        
        # Parse material
        material_match = re.search(r'(PVC|CONCRETE|DUCTILE|CAST|HDPE|STEEL|CLAY|VITRIFIED)', text_content, re.IGNORECASE)
        if material_match:
            ...
            if material_name == "PVC":
                ...
                labels["material"] = Material.CLAY
            elif material_name == "VITRIFIED":
                labels["material"] = Material.VITRIFIED_CLAY
        
        # Parse invert elevation (e.g., "INV 100.5", "INVERT 100.5")
        invert_match = re.search(r'INV(?:ERT)?\s*(\d+(?:\.\d+)?)', text_content, re.IGNORECASE)
        if invert_match:
            labels["invert_ft"] = float(invert_match.group(1))
        
        # Parse rim elevation (e.g., "RIM 102.5", "RIM ELEV 102.5")
        rim_match = re.search(r'RIM(?:\s*ELEV)?\s*(\d+(?:\.\d+)?)', text_content, re.IGNORECASE)
        if rim_match:
            labels["rim_ft"] = float(rim_match.group(1))
