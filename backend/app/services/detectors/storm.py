"""Storm water network detection and extraction."""
import re
import math
import uuid
...
def detect_nodes(vectors: List[VectorEl], texts: List[TextEl], symbol_map: Dict[str, Any]) -> List[Node]:
    """
    Detect storm water network nodes from vectors and texts.
    ...
    # Get storm-related symbols from symbol map
    storm_symbols = _get_storm_symbols(symbol_map)
    
    # Detect manholes and catch basins
    for vector in vectors:
        if _is_storm_node(vector, storm_symbols):
    ...
    # Detect inlets
    for vector in vectors:
        if _is_inlet(vector, storm_symbols):
            node = _create_node_from_vector(vector, NodeType.INLET)
            if node:
                nodes.append(node)
    
    # Detect junctions (intersections of storm lines)
    junctions = _detect_junctions(vectors, storm_symbols)
    nodes.extend(junctions)
    
    logger.info(f"Detected {len(nodes)} storm water nodes")
    return nodes


def trace_edges(vectors: List[VectorEl], symbol_map: Dict[str, Any]) -> List[Edge]:
    """
    Trace storm water network edges from vectors.
    ...
    # Find storm pipe vectors
    storm_pipes = [v for v in vectors if _is_storm_pipe(v, storm_symbols)]
    
    # Merge connected polylines
    merged_pipes = _merge_connected_polylines(storm_pipes)
    ...
    logger.info(f"Traced {len(edges)} storm water edges")
    return edges


def attach_labels(edges: List[Edge], texts: List[TextEl]) -> List[Edge]:
    """
    Attach labels to storm water edges.
    ...
    for edge in edges:
        ...
        parsed_labels = _parse_storm_labels(nearby_texts)
...
def _get_storm_symbols(symbol_map: Dict[str, Any]) -> Dict[str, Any]:
    """Get storm-related symbols from symbol map."""
    storm_symbols = {}
    
    if "symbols" not in symbol_map:
        return storm_symbols
    
    symbols = symbol_map["symbols"]
    
    # Look for storm-related symbols
    storm_keywords = ["storm", "drain", "catch", "inlet", "manhole", "pipe", "conduit"]
    
    for symbol_name, symbol_data in symbols.items():
        symbol_name_lower = symbol_name.lower()
        layer_hints = symbol_data.get("layer_hint", [])
        
        # Check if symbol is storm-related
        is_storm = any(keyword in symbol_name_lower for keyword in storm_keywords)
        is_storm_layer = any("storm" in layer.lower() for layer in layer_hints)
        
        if is_storm or is_storm_layer:
            storm_symbols[symbol_name] = symbol_data
    
    return storm_symbols


def _is_storm_node(vector: VectorEl, storm_symbols: Dict[str, Any]) -> bool:
    """Check if vector represents a storm node."""
    # Check against storm symbols
    for symbol_name, symbol_data in storm_symbols.items():
        if _matches_symbol(vector, symbol_data):
            return True
    
    # Check for common storm node patterns
...
def _is_inlet(vector: VectorEl, storm_symbols: Dict[str, Any]) -> bool:
    """Check if vector represents an inlet."""
    # Check against storm symbols for inlets
    for symbol_name, symbol_data in storm_symbols.items():
        if "inlet" in symbol_name.lower() and _matches_symbol(vector, symbol_data):
            return True
    
    # Check for inlet patterns (rectangular with specific proportions)
    if vector.kind == "rect":
        ...
        if bbox:
            ...
            # Inlets typically have aspect ratio between 1.5 and 3.0
...
def _is_storm_pipe(vector: VectorEl, storm_symbols: Dict[str, Any]) -> bool:
    """Check if vector represents a storm pipe."""
    # Check against storm symbols
    for symbol_name, symbol_data in storm_symbols.items():
...
def _detect_junctions(vectors: List[VectorEl], storm_symbols: Dict[str, Any]) -> List[Node]:
    """Detect junctions where storm lines intersect."""
    junctions = []
...
def _parse_storm_labels(texts: List[TextEl]) -> Dict[str, Any]:
    """Parse storm water labels from text elements."""
    labels = {}
    ...
    for text in texts:
        ...
        # Parse material
        material_match = re.search(r'(PVC|CONCRETE|DUCTILE|CAST|HDPE|STEEL|CLAY)', text_content, re.IGNORECASE)
        if material_match:
