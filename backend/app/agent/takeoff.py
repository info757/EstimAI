...
from backend.app.services.detectors.legend import (
    find_legend_regions,
    sample_symbol_snippets,
)
from backend.app.services.detectors.qa_rules import validate_network_qa
from backend.app.services.detectors.sanitary import (
    attach_labels as sanitary_attach_labels,
)
from backend.app.services.detectors.sanitary import (
    detect_nodes as sanitary_detect_nodes,
)
from backend.app.services.detectors.sanitary import trace_edges as sanitary_trace_edges
from backend.app.services.detectors.sitework import (
    count_inlet_protections,
    measure_curb_lf,
    measure_sidewalk_sf,
    measure_silt_fence_lf,
)
from backend.app.services.detectors.storm import (
    attach_labels,
    detect_nodes,
    trace_edges,
)
from backend.app.services.detectors.symbol_map import build_symbol_map_via_llm
from backend.app.services.detectors.water import attach_labels as water_attach_labels
from backend.app.services.detectors.water import detect_nodes as water_detect_nodes
from backend.app.services.detectors.water import trace_edges as water_trace_edges
...
from backend.app.services.ingest.pdfnet_runtime import iter_pages, open_doc
...
class TakeoffSession:
    ...
    created_at: datetime
    updated_at: datetime
    result: EstimAIResult | None = None
...
class TakeoffRequest:
    ...
    upload_file: Any | None = None  # FastAPI UploadFile
...
class TakeoffResponse:
    ...
    session_id: str
    status: str
    proposed_review: EstimAIResult | None = None
...
class TakeoffAgent:
    ...
    async def process_takeoff(self, request: TakeoffRequest) -> TakeoffResponse:
        ...
        try:
            ...
            logger.error(f"Takeoff processing error for session {session_id}: {e!s}")
            if session_id in self.sessions:
    ...
    async def _process_with_retries(self, session: TakeoffSession, request: TakeoffRequest) -> EstimAIResult:
        ...
        for attempt in range(session.max_retries + 1):
            try:
                ...
                logger.warning(f"Takeoff attempt {attempt + 1} failed for session {session.session_id}: {e!s}")
    ...
    async def _run_takeoff_pipeline(self, session: TakeoffSession, request: TakeoffRequest) -> EstimAIResult:
        ...
        try:
            ...
            raise Exception(f"Failed to open PDF: {e!s}")
...
def get_session_status(session_id: str) -> TakeoffSession | None:
